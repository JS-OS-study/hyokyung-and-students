...
컴퓨터가 부팅되어 운영체제가 실행된 후 최초의 프로그램을 실행할 때 다음과 같은 과정이 발생한다.
...
위 내용은 대략적인 예시이며, 실제로 리그오브레전드를 복사해서 디아블로를 실행한다는 것은 아니다.

리그오브레전드나 디아블로를 실행하기 위해 복사하는 상위의 어떤 프로세스가 있을것이라는 의미이다.
...
`exit()` 는 프로세스가 종료 될 때 운영체제에게 자신의 작업이 끝났음을 알리는 `자발적 종료(Non-Preemptive`)에 해당하는 시스템 콜이다.
